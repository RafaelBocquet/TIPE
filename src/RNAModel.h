    ...
    /* The first vector can be computed faster than others as most in_vec components are zero */
    {
      ...
      iterateOnContext([&](unsigned i){
        for(unsigned j = 0; j < LayerSizes[1]; ++j){
          cacheResult[1].at(j, 0) += layers[0].at(j, i);
        }
      });
      cacheDerivative[1].init([&](unsigned i, unsigned j) -> FixedPoint20 {
        return activation_derivative(cacheResult[1].at(i, j));
      });
      cacheResult[1].apply([&](unsigned, unsigned, FixedPoint20 const& v) -> FixedPoint20 {
